...
BADCC1=("gcc-5.1.0 -m32 -O3 -g")
BADCC2=() # exec failures 
BADCC3=() # wrong results 
GOODCC=("gcc-4.9.0 -m32 -O3 -g")
TIMEOUTCC=20
TIMEOUTEXE=2
TIMEOUTCCOMP=20
CFLAG="-o t"
CLANGFC="clang-3.6.0 -m32 -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined"
CHECKCCOMP=1
...
for cc in "${BADCC1[@]}" ; do 
    ...
    if ! grep 'internal compiler error: in simplify_subreg, at simplify-rtx.c' out2.txt ; then 
    exit 1
    fi
done
