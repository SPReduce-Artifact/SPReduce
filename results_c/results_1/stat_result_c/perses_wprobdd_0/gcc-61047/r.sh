...
BADCC2=("gcc-4.9.0 -m32 -O1")
BADCC3=()
GOODCC=("gcc-7.1.0 -m32 -O1")
TIMEOUTCC=6
TIMEOUTEXE=2
TIMEOUTCCOMP=10
CFLAG="-o t"
CLANGFC="clang-trunk -m64 -O0 -Wall -fwrapv -ftrapv -fsanitize=undefined"
...
if 
  timeout -s 9 $TIMEOUTCC clang-trunk -pedantic -Wall -Wsystem-headers -O0 -c $CFILE  >out.txt 2>&1 &&\
...
#  ! grep 'uninitialized' out.txt &&\
  ...
  timeout -s 9 $TIMEOUTCC gcc-trunk -Wall -Wextra -Wsystem-headers -O0 $CFILE >outa.txt 2>&1 &&\
