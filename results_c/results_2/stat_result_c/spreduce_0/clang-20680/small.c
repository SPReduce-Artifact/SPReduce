 char(safe_div_func_int8_t_s_s)(char si1, char si2) {
  return
      si2 == 0 ? 0 :
                                                           si1 / si2;
}
 int g_4, g_185, func_65(unsigned char p_66, int p_67) ;
  char g_27[7] ;
static unsigned char func_85(unsigned char p_86, int p_87, char p_88);
 int func_1() {
  int l_10[7];
  g_27[6] ^=
      g_4;
    if (func_65(
                                                           0,
                                                           g_27[0])) {
    }
  return 0;
}
 int func_65(unsigned char p_66, int p_67) {
  int l_311 = 0xE02EBC21L ;
  for (p_66 = -14; p_66 >= 51; ++p_66) {
    char l_241 = 0xAAL;
      func_85(0,
                                          g_27[6],
                                          0);
        if (p_67 |
             p_66) func_85(0,
                                              g_27[6],
                                              0);
        for (; l_241; ++l_241) {
          l_311 &=
              safe_div_func_int8_t_s_s(
                   0xB2L,
                   p_66);
          int i;
            for (; i; i++)
              {}
        }
  }
  return 0;
}
 unsigned char func_85(unsigned char p_86, int p_87, char p_88) {
  int l_91 = 4294967295UL;
  if (p_87) {
     char l_100[4][5] ;
    int l_120 ;
      for (; p_87 < 25; ++p_87) {
         int l_105[7] = {};
        for (; l_91; l_91 += 1) l_120 =
              l_91 > 0 ||
       l_105[l_91] && l_120 <= 0 && l_100[0][l_91] ||
       l_91 << l_91 !=
                                           0 ^
                                         l_120 && 0 < 32767-l_91 << l_100[0][l_91] ^
                                         l_120 ||
       l_120 <= 0 && l_105[l_91] && l_91 << l_100[0][l_91] !=
                                           0 ^
                                         0 < 32767 / l_91;
      }
  }g_185 = 2L;
  return 6;
}
int main() {
  func_1();
}
