...
class="compiler.runtime.StackOverflowBug"
...
mkdir -p compiler/runtime
cp StackOverflowBug.java compiler/runtime/StackOverflowBug.java
python3 $javaMender $script_dir/compiler/runtime/StackOverflowBug.java
if ! /01JVMS/linux64/openjdk11/openj9_11/bin/javac -cp $classpath compiler/runtime/StackOverflowBug.java
then
        exit 1
fi
