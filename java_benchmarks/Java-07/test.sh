#!/usr/bin/env bash
CFILE="TestSquareToLen.java"
script_dir=$(dirname "$(realpath "$0")")

class="compiler.intrinsics.bigInteger.TestSquareToLen"
...
mkdir -p compiler/intrinsics/bigInteger
cp TestSquareToLen.java compiler/intrinsics/bigInteger/TestSquareToLen.java
python3 $javaMender $script_dir/compiler/intrinsics/bigInteger/TestSquareToLen.java
if ! /01JVMS/linux64/openjdk11/openj9_11/bin/javac -cp $classpath compiler/intrinsics/bigInteger/TestSquareToLen.java
then
        exit 1
fi
# __MARK:C__
