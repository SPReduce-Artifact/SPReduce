#!/usr/bin/env bash
CFILE="TestMulAdd.java"
script_dir=$(dirname "$(realpath "$0")")

class="compiler.intrinsics.bigInteger.TestMulAdd"
...
cp TestMulAdd.java compiler/intrinsics/bigInteger/TestMulAdd.java
python3 $javaMender $script_dir/compiler/intrinsics/bigInteger/TestMulAdd.java
if ! /01JVMS/linux64/openjdk11/openj9_11/bin/javac -cp $classpath compiler/intrinsics/bigInteger/TestMulAdd.java
then
        exit 1
fi
# __MARK:C__
